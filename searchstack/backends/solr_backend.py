...
from ..models import SearchResult, SearchResultGroup
...
class SolrSearchBackend(BaseSearchBackend):
    ...
    def _process_docs(self, raw_results, docs, result_class, distance_point, hits):
        from .. import connections
    ...
    def _process_results(self, raw_results, highlight=False, result_class=None,
        ...
        if group_field is not None and hasattr(raw_results, 'grouped'):
            ...
            for group in grouped['groups']:
                ...
                group_docs, group_hits = self._process_docs(raw_results, doc_list['docs'],
                                                            result_class, distance_point,
                                                            doc_list['numFound'])
                result_group = SearchResultGroup(group['groupValue'], group_docs, group_hits)
                results.append(result_group)
        else:
            results, hits = self._process_docs(raw_results, raw_results.docs, result_class,
                                               distance_point, hits)
