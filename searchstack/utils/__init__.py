...
def default_get_identifier(obj_or_string):
    ...
    return "%s.%s" % (get_model_ct(obj_or_string),
...
def _lookup_identifier_method():
    ...
    If SEARCHSTACK_IDENTIFIER_METHOD is not defined, return searchstack.utils.default_get_identifier.
    ...
    try:
        ...
        raise ImportError("Unable to import module '%s' provided for SEARCHSTACK_IDENTIFIER_METHOD." % module_path)

    identifier_method = getattr(module, method_name, None)

    if not identifier_method:
        raise AttributeError(
            "Provided method '%s' for SEARCHSTACK_IDENTIFIER_METHOD does not exist in '%s'." % (method_name, module_path)
        )
