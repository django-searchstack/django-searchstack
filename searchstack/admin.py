...
class SearchChangeList(ChangeList):
    def __init__(self, **kwargs):
        self.searchstack_connection = kwargs.pop('searchstack_connection', 'default')
    ...
    def get_results(self, request):
        ...
        sqs = SearchQuerySet(self.searchstack_connection).models(self.model).auto_query(request.GET[SEARCH_VAR]).load_all()

        paginator = Paginator(sqs, self.list_per_page)
        # Get the number of objects, with admin filters applied.
        result_count = paginator.count
        full_result_count = SearchQuerySet(self.searchstack_connection).models(self.model).all().count()
...
class SearchModelAdminMixin(object):
    ...
    searchstack_connection = 'default'
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        # returned bits.
        if self.model not in connections[self.searchstack_connection].get_unified_index().get_indexed_models():
        ...
        kwargs = {
            'searchstack_connection': self.searchstack_connection,
            'request': request,
            'model': self.model,
