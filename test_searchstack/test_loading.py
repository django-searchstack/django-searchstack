...
from searchstack.exceptions import NotHandled, SearchFieldError
...
class ConnectionHandlerTestCase(TestCase):
    ...
    def test_get_item(self):
        ...
        self.assertEqual(backend_path, 'searchstack.backends.solr_backend.SolrEngine')

        solr_engine_2 = ch['default']
        backend_path_2, memory_address_2 = repr(solr_engine_2).strip('<>').split(' object at ')
        self.assertEqual(backend_path_2, 'searchstack.backends.solr_backend.SolrEngine')
    ...
    def test_get_unified_index(self):
        ...
        self.assertEqual(str(klass), 'searchstack.utils.loading.UnifiedIndex')

        ui_2 = ch['default'].get_unified_index()
        klass_2, address_2 = repr(ui_2).strip('<>').split(' object at ')
        self.assertEqual(str(klass_2), 'searchstack.utils.loading.UnifiedIndex')
...
class ConnectionRouterTestCase(TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual([str(route.__class__) for route in cr.routers], ["<class 'searchstack.routers.DefaultRouter'>"])

    @override_settings(SEARCHSTACK_ROUTERS=['searchstack.routers.DefaultRouter'])
    ...
    @override_settings(SEARCHSTACK_ROUTERS=['test_searchstack.mocks.MockMasterSlaveRouter', 'searchstack.routers.DefaultRouter'])
    def test_router_override3(self):
        cr = loading.ConnectionRouter()
        self.assertEqual([str(route.__class__) for route in cr.routers], ["<class 'test_searchstack.mocks.MockMasterSlaveRouter'>", "<class 'searchstack.routers.DefaultRouter'>"])
    ...
    @override_settings(SEARCHSTACK_ROUTERS=['test_searchstack.mocks.MockPassthroughRouter', 'test_searchstack.mocks.MockMasterSlaveRouter', 'searchstack.routers.DefaultRouter'])
