...
from searchstack import connections, indexes, reset_search_queries
from searchstack.exceptions import SkipDocument
from searchstack.inputs import AltParser, AutoQuery, Raw
...
class SolrSearchBackendTestCase(TestCase):
    ...
    def test_non_silent(self):
        bad_sb = connections['solr'].backend('bad', URL='http://omg.wtf.bbq:1000/solr',
    ...
    def test_spatial_search_parameters(self):
        ...
        # See http://django-searchstack.readthedocs.org/en/latest/spatial.html#points
...
class LiveSolrSearchQuerySetTestCase(TestCase):
    ...
    def test_related_load_all(self):
        ...
        self.assertEqual(sqs[0].object.foo,
                         ...
                         "future-dated or non-live content is not indexed and searchable. "
                         "Our ``Note`` model has a ``pub_date`` field, so let's update our code "
                         "to include our own ``SearchIndex`` to exclude indexing future-dated notes:")
...
class LiveSolrMoreLikeThisTestCase(TestCase):
    ...
    def test_more_like_this_defer(self):
        ...
        # FIXME: this currently is known to fail because searchstack.utils.loading doesn't see the
