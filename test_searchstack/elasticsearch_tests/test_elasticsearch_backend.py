...
class ElasticsearchFacetingMockSearchIndex(indexes.SearchIndex, indexes.Indexable):
    text = indexes.TextField(document=True)
    author = indexes.TextField(model_attr='author', faceted=True)
    editor = indexes.TextField(model_attr='editor', faceted=True)
    pub_date = indexes.DateField(model_attr='pub_date', faceted=True)
    facet_field = indexes.CharField(model_attr='author')
...
class TestSettings(TestCase):
    ...
    def test_kwargs_are_passed_on(self):
        from searchstack.backends.elasticsearch_backend import ElasticsearchSearchBackend
...
class FailedElasticsearchSearchBackendTestCase(TestCase):
    def setUp(self):
        ...
        logging.getLogger('searchstack').addHandler(self.cap)
        import searchstack
        logging.getLogger('searchstack').removeHandler(searchstack.stream)
    ...
    def tearDown(self):
        ...
        connections['elasticsearch']._index = self.old_ui
        logging.getLogger('searchstack').removeHandler(self.cap)
        logging.getLogger('searchstack').addHandler(searchstack.stream)
